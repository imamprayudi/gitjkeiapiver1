...
<!DOCTYPE html>
  ...
  <body>
    ...
    DELIVERY INSTRUCTIONS EDIT<br /><br />
    ...
    <form id="formdi">
      ...
      <input type="submit" value="EDIT DI">
    </form>
    <br />
...
<div class="mt-3" id="boxhasil" style="display:none;">
  <h5></h5>
<div class="mb-2">
    <input type="text" id="invoiceglobal"
           class="form-control form-control-sm d-inline-block"
           style="width:250px"
           placeholder="Update Invoice">

    <button type="button"
            class="btn btn-success btn-sm"
            onclick="updateSelectedInvoice()">
        Update Invoice
    </button>
</div>

<div class="mb-2">
    <input type="number" id="qtyglobal"
           class="form-control form-control-sm d-inline-block"
           style="width:150px"
...
async function updateSelectedInvoice()
{
    const invoice = document.getElementById("invoiceglobal").value.trim();

    if(invoice==''){
        alert("Isi invoice dulu");
        return;
    }

    /* INVOICE TIDAK BOLEH LEBIH DARI 15 KARAKTER */
    if(invoice.length > 15)
    {
      alert("Invoice maksimal 15 karakter");
      document.getElementById("invoiceglobal").focus();
      return;
    }

    const checks = document.querySelectorAll(".pilihrow:checked");
    ...
    for(const chk of checks)
    {
        ...
        try{
            ...
            await fetch('http://136.198.117.117/api/apijkeidev/apidiinv.php',{
                method:'POST',
                headers:{
    ...
    document.getElementById("formdi").dispatchEvent(
        new Event('submit')
    );
}

async function updateSelectedQty()
{
    const qty = document.getElementById("qtyglobal").value.trim();
    ...
    for(const chk of checks)
    {
        ...
        try{
            ...
            await fetch(
            'http://136.198.117.117/api/apijkeidev/apidiqty.php',
            {
                method:'POST',
                headers:{
                    'Content-Type':'application/x-www-form-urlencoded'
    ...
    document.getElementById("formdi")
    .dispatchEvent(new Event('submit'));
}

async function uploadSelectedStatus()
{
    const checks = document.querySelectorAll(".pilihrow:checked");
    ...
    if(!confirm("Upload data terpilih ?"))
    {
        return;
    }
    ...
    for(const chk of checks)
    {
        ...
        try{
            ...
            await fetch(
            'http://136.198.117.117/api/apijkeidev/apidiupload.php',
            {
                method:'POST',
                headers:{
                    'Content-Type':'application/x-www-form-urlencoded'
