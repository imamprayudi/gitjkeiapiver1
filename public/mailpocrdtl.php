...
$pono = base64_decode(urldecode($p));

if ($pono == '') {
    die('PO Number tidak ditemukan');
}
...
.then(async data => 
{
  ...
  // await updateReadStatus(); tidak perlu karena ini detail per PO, bukan list PO
  loadData();
}) 
...
async function loadData()
{
  try
  {
    const response = await fetch('../api/apimailpocrdtl.php', {
      method: "POST",
      headers: {
