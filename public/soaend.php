...
.then(data => 
{
  ...
  urlsoaend = data.urlsoaend;
  urlsoacomend = data.urlsoacomend;
  getSupplier(user);
  getTanggal(user);
}) 
...
// ------------------------------------------------------------------------
async function getSoaend(supp,tgl)
{
  let supplier = supp;
  let tanggal = tgl;
  try 
  {
    const response = await fetch(urlsoaend, 
    {
      method: 'POST',
      credentials: "include",
      headers: {
...
async function postCom(blnthn,supp,suppkomen,jeinkomen)
{
  try 
  {
    const response = await fetch(urlsoacomend, 
    {
      method: 'POST',
      credentials: "include",
      headers: {
